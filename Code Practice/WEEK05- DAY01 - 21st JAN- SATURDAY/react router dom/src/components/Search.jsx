...
const Search = ({ allMeals, setFilteredMeals }) => {
  const [searchText, setSearchText] = useState("");

  function handelSearch() {
    const filteredData = allMeals.filter((data) =>
      data.strMeal.toLowerCase().includes(searchText.toLowerCase())
    );
    return filteredData;
  }
  ...
  return (
    <div className="flex w-full max-w-sm items-center space-x-2 my-5">
      ...
      <button
        ...
        onClick={() => {
          const filteredMeals = handelSearch();
